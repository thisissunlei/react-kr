...
export default class ReportDetail extends React.Component{
	...
	onExportSign=(values)=>{
		...
		var url = location.protocol +"//"+`op.krspace.cn/api/krspace-finance-web/csr/source/stat/export/type/sign?${where.join('&')}`
	...
	onExportAdd=(values)=>{
		...
		var url = location.protocol +"//" + `op.krspace.cn/api/krspace-finance-web/csr/source/stat/export/type/add?${where.join('&')}`
		window.location.href = url;
	}
