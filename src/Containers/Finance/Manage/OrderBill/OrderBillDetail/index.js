...
export default class AttributeSetting extends React.Component {
    ...
    openReceivedBtn() {
        ...
        Store.dispatch(Actions.callAPI('getPaymentActData', {mainbillId: _this.props.params.orderId})).then(function(response) {
            ...
            _this.setState({payWayList, accountDetail: response.propData, contractTopReceive: response.contract, contractReceive});
        }).catch(function(err) {
    ...
    openSwitchBtn() {
        ...
        if (this.state.listValues.length == 0) {
            ...
            Store.dispatch(Actions.callAPI('findContractListById', {mainbillId: _this.props.params.orderId})).then(function(response) {
                ...
                _this.setState({receivedList: receivedList});
            }).catch(function(err) {
                Message.error(err.message);
            });
        }
    ...
    openAccountBtn() {
        ...
        Store.dispatch(Actions.callAPI('getOnNewAccountData', {mainbillId: _this.props.params.orderId})).then(function(response) {
            ...
            _this.setState({payWayList, accountDetail: response.propData, contractList, stationPayment: stationPayment});
        }).catch(function(err) {
            Message.error(err.message);
        });
        this.setState({
    ...
    onSearch(params) {
        ...
        this.setState({params, listValues: listValues});
...
//1 押金 item.depositId   2  定金  item.frontId  3工位服务费  item.totalrentId
    ...
    initBasicInfo() {
        ...
        Store.dispatch(Actions.callAPI('getAccountFlow', {mainbillid: params.orderId})).then(function(response) {
            _this.setState({
                ...
                colorClassName: response.isIncomeRunning == 2
                    ? 'historyIncomeGray'
                    : 'historyIncome'
            });
        }).catch(function(err) {
    ...
    historyIncomed = () => {
        ...
        if (isRunningIncome == 0) {
            ...
            Store.dispatch(Actions.callAPI('runStationIncome', {mainbillId: params.orderId})).then(function(response) {
                ...
                Message.error(err.message);
                _this.setState({isRunningIncome: 0});
            });
        }
    }
    ...
    initializeSnack = (open = false, title = '正在补历史收入...', titleAfter, color) => {
        ...
        return (<SnackTip style={style} open={open} title={title} titleAfter={titleAfter} onClose={this.snackTipClose}/>);
    }
    ...
    renderReceived = () => {
        return (
            <Table style={{
                marginTop: 60
            }} ajax={true} loading={this.state.isLoading} onSelect={this.onSelect} onLoaded={this.onLoaded} ajaxUrlName='getAccountNewFlow' ajaxParams={this.state.params} onOperation={this.onOperation}>
                <TableHeader>
                ...
                <TableBody>
                    <TableRow>
                        <TableRowColumn name="tradingCode" style={{
                            width: 160,
                            overflow: "visible"
                            ...
                            return (
                                <div style={{
                                    display: TooltipStyle,
                                    paddingTop: 5
                                }} className='financeDetail-hover'>
                                    <span className='tableOver' style={{
                                        maxWidth: 160,
                        ...
                        <TableRowColumn name="finaflowdesc" style={{
                            width: 160,
                            overflow: "visible"
                        }} component={(value, oldValue) => {
                            var TooltipStyle = "";
    ...
    render() {
        ...
        const close = [< Button label = "关闭" joinEditForm style = {{marginLeft:10}}
            onTouchTap = {
        ...
        return (
            ...
            <div className='m-orderBillDetail'>
                ...
                <Section title="订单明细账" description="" style={{
                    marginBottom: -5,
                    minHeight: 910
                }}>

                    <DotTitle title='订单描述' style={{
                        marginTop: '6',
                        marginBottom: '40'
                    }}/>
                    <BasicInfo detail={this.state.basicInfo}/>

                    <DotTitle title='订单明细账' style={{
                        marginTop: '28',
                        marginBottom: '62'
                    }}/>
                ...
                <Dialog title="高级查询" open={this.state.openSearch} onClose={this.closeSearchDialog} contentStyle ={{
                    width: '686'
                }}>
                ...
                <Drawer open={this.state.openRight} width={650} openSecondary={true} className='m-finance-drawer' containerStyle={{
                    top: 60,
                    paddingBottom: 228,
                    zIndex: 20
                }}>
                    <div>
                        ...
                        <ReceivedBtnForm open={this.state.openRight} onSubmit={this.onAddReceivedSubmit} onCancel={this.iconClose} optionList={this.state.payWayList} accountDetail={this.state.accountDetail} contractReceive={this.state.contractReceive} calcBalance={this.calcBalance}/>
                    </div>
                </Drawer>

                {this.state.openRight &&< div className = 'trasparent' onClick = {
                    this.iconClose
                } > </div>}

                <Dialog title="退款" open={this.state.openQuitBtn} onClose={this.closeQuitBtn} contentStyle ={{
                    width: '688'
                }}>
                    <QuitBtnForm onSubmit={this.onQuitSubmit} onCancel={this.closeQuitBtn} fiMoney={fiMoney} initialValuesId={initialValuesId}/>
                </Dialog>

                <Dialog title="添加转移" open={this.state.openShift} onClose={this.closeShiftBtn} contentStyle ={{
                    width: '688'
                }}>
                ...
                <Dialog title="转押金" open={this.state.openSwitchBtn} onClose={this.closeSwitchBtn} contentStyle ={{
                    width: '688'
                }}>
                ...
                <Dialog title="转营收" open={this.state.openBusinessBtn} onClose={this.closeBusinessBtn} contentStyle ={{
                    width: '688'
                }}>
                ...
                <Dialog title="挂账" open={this.state.openAddaccountBtn} onClose={this.closeAddaccount} contentStyle ={{
                    width: '688'
                }}>
                ...
                <Dialog title="补收入" open={this.state.openSupplementBtn} onClose={this.openSupplementBtn} contentStyle ={{
                    width: '445',
                    height: '236'
                }}>
                ...
                <Dialog title="查看" open={this.state.openView} onClose={this.closeViewDialog}>
                    <ViewForm detail={this.state.itemDetail}/>
                </Dialog>

                <Dialog title="合同详情" open={this.state.openContract} onClose={this.contractContinue} contentStyle ={{
                    width: '443',
                    height: '236'
                }}>
