...
@observer
 class CardTwo extends  React.Component{
	...
	onSubmit = (values) => {
        ...
        // values.wherefloors=JSON.stringify(values.wherefloors);
	...
	onCancel = () => {
        ...
        Store.dispatch(change('CardTwo','local',State.detailData.longitude+','+State.detailData.latitude)); 
    }
  ...
  communityCodeChange=(value)=>{
   ...
   editChange=()=>{
    ...
    console.log(showEdit,'==render',State.cardTwoEdit)

		return (
            <div >
              <form  onSubmit={handleSubmit(this.onSubmit)} >
                    <div>
							<Grid style={{marginBottom:20,display:showEdit?'block':'none'}}>
								<Row>
							...
							<Grid style={{marginBottom:20,display:showEdit?'none':'block'}}>
								<Row>
									<Col md={12} align="right">
										<Button  label="编辑" type="button" onTouchTap={this.editChange}/>
											
									</Col>
								</Row>
							</Grid>
					</div>
				<div className="watch" style={{display:showEdit?'none':'block'}}>
                    <div >
                ...
                <div className="edit-form" style={{display:showEdit?'block':'none'}}>
