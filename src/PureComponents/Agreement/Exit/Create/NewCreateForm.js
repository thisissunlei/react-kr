...
<form className="exit-dialog-form" onSubmit={handleSubmit(this.onSubmit)}  style={{marginTop:50}}>
			<div className="cheek" style={{paddingLeft:0,marginLeft:23}}>
				...
				<KrField className="red-label" style={{width:262,marginLeft:25}} name="withdrawdate" component="date" label="退租日期（当日不收租金，若无法确认，请联系技术部-王鹏）" requireLabel={true}  onChange={this.setTotalRent}/>
				...
				<KrField name="totalreturn" style={{width:262,marginLeft:25}} type="text" component="labelText" label="退服务费总额"
				requireLabel={true} requiredValue={true} pattern={/^\d{0,16}(\.\d{0,2})?$/} errors={{requiredValue:'退服务费总额为必填项',pattern:'请输入正数金额，小数点后最多两位'}} 
...
const validate = values => {
	...
	if (values.totalreturn && isNaN(values.totalreturn)) {
		errors.totalreturn = '退服务费总额必须为数字';
	}
